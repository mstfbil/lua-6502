...
_M.getParam = {
	[_M.addrmode.ILLEGAL] = function(self)
		return nil
	end,
	[_M.addrmode.IMP] = function(self)
		return nil
	end,
	[_M.addrmode.ACC] = function(self)
		return nil
	end,
	...
	[_M.addrmode.ABS] = function(self)
		...
		local p = bor(self:readmem(self.pc), lshift(self:readmem(band(self.pc + 1, 0xFFFF)), 8))
	...
	[_M.addrmode.ABI] = function(self)
		-- Absolute indirect
		local a = bor(self:readmem(self.pc), lshift(self:readmem(band(self.pc + 1, 0xFFFF)), 8))
		self.pc = band(self.pc + 2, 0xFFFF)
		local param = a
		param = bor(self:readmem(a), lshift(self:readmem(band(a + 1, 0xFFFF)), 8))
		return param
	end,
	...
	[_M.addrmode.ABY] = function(self)
		...
		local param = bor(self:readmem(self.pc), lshift(self:readmem(band(self.pc + 1, 0xFFFF)), 8)) + self.Y
		self.pc = band(self.pc + 2, 0xFFFF)
		return param
	end,
...
_M.operations = {
	...
	[_M.optype.NOP] = function(self)
		return 0
	end,
	[_M.optype.WAI] = function(self)
		return 0
	end,
	...
	[_M.optype.EOR] = function(self, param)
		self.A = bxor(self.A, self:readmem(param))
		self:setnz(self.A)
		return 0
	end,
	...
	[_M.optype.SBC] = function(self, param)
		...
		local B = bxor(self:readmem(param), 0xFF)
		...
		if band(self.F, self.flags.D) == 0x00 then
			...
			Aout = self.A + B + Cin
			Vout = band(bxor(self.A, Aout), bxor(B, Aout), 0x80)
	...
	[_M.optype.ADC] = function(self, param)
		...
		if band(self.F, self.flags.D) == 0x00 then
			...
			Aout = self.A + B + Cin
			-- Overflow logic using bitwise operations
			Vout = band(bxor(self.A, Aout), bxor(B, Aout), 0x80)
			...
			if band(bxor(self.A, B), 0x80) == 0x00 then
				...
				if band(bxor(self.A, Aout), 0x80) ~= 0x00 then
...
function _M:nmi()
	self.F = band(self.F, bnot(self.flags.B)) -- clear Break flag
